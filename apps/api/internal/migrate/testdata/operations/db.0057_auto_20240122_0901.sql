-- Rows for db.0057's create_notification_preferences to act on.
--
-- It gives a row to every user who is not a bot, so one of the pair below is one. The five preference flags are the model's defaults rather than anything read off the user.
--
-- UNSTABLE: user_notification_preferences.id, user_notification_preferences.created_at, user_notification_preferences.updated_at
