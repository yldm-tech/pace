...
func TestAnUpdateWithNoFieldsStillTouchesTheRow(t *testing.T) {
	...
	updates := map[string]any{"updated_at": now, "updated_by_id": "actor-id"}
	if len(updates) != 2 {
...
func TestTheStrippedDescriptionIsRecomputed(t *testing.T) {
	...
	if err := ApplyUpdate(nil, updates, "project-id", nil, "<p>before</p>", time.Now()); err != nil {
		t.Fatal(err)
	}
	...
	if err := ApplyUpdate(nil, updates, "project-id", nil, "", time.Now()); err != nil {
		t.Fatal(err)
	}
...
func TestCompletedAtFollowsAStateChange(t *testing.T) {
	...
	if err := syncCompletedAt(nil, updates, &stored, now); err != nil {
		t.Fatal(err)
	}
...
func TestClearingTheStateFallsBackToTheDefault(t *testing.T) {
	...
	if err := ensureState(nil, untouched, "project-id"); err != nil {
		t.Fatal(err)
	}
	...
	if err := ensureState(nil, absent, "project-id"); err != nil {
		t.Fatal(err)
	}
	if len(absent) != 0 {
