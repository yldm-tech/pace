...
// composeServices reads the service names out of the repository's compose file. A regex rather than a YAML parser, so that the test adds no dependency for one list of keys: service names are the only two-space-indented keys under `services:`.
func composeServices(t *testing.T) map[string]bool {
	...
	start := regexp.MustCompile(`(?m)^services:\s*$`).FindStringIndex(body)
	if start == nil {
		t.Fatalf("%s has no services block", composePath)
	}
	services := map[string]bool{}
	for _, match := range regexp.MustCompile(`(?m)^  ([a-z][a-z0-9-]*):\s*$`).FindAllStringSubmatch(body[start[1]:], -1) {
		services[match[1]] = true
	}
	if len(services) == 0 {
		t.Fatalf("%s declares no services", composePath)
	}
	return services
}
