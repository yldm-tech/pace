...
```bash
python ../api/tools/generate_relation_graph.py > ../api/internal/worker/relation_graph.json
```
...
`docker-compose.yml` still has a service called `live`, and the container is still called `plane-live`. Only the build changed: `./apps/api` with `entrypoint: ["pace-live"]` instead of `./apps/live/Dockerfile.live`. The names are deliberate. `apps/proxy/Caddyfile.ce` routes with `reverse_proxy /live/* live:3000`, and the web and admin apps are handed a `LIVE_URL`; renaming the service would mean editing both, so it keeps its name and neither changes.
...
`apps/api/bin` now mirrors `apps/api/bin`, one script per service, shipped into the image as `pace-entrypoint-*`. The compose file, the CLI deployment and the all-in-one image all call those rather than each restating the sequence. `collectstatic` has no counterpart: Django ran it to lay out what whitenoise serves under `/static/`, and the Go service has none.
...
- **`apps/api/.env`** — the file every compose file read. It is `apps/api/.env` now, and `setup.sh` writes it there.
