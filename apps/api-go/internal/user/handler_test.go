...
func TestUserRouteInventory(t *testing.T) {
	...
	router := gin.New()
	NewHandler(nil, sessions, fakeUsers{}, Settings{}).Register(router)
	// Twenty-three: fifteen of the person's own routes, three for a profile image and five for the keys they have made. The external API's /api/v1/users/me/ is not among them — it is served by internal/externalapi, because it authenticates with a key rather than a session.
	if got := len(router.Routes()); got != 23 {
		t.Fatalf("user routes = %d, want 23", got)
	}
