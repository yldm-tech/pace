...
CPU_ARCH=$(uname -m)
IMAGE_NAME=${IMAGE_NAME:-${PLANE_IMAGE_OWNER:-ghcr.io/yldm-tech}/pace-aio-community}
...
if [ -z "$APP_RELEASE_VERSION" ]; then
    ...
    echo ""
    echo "Flags:"
    echo "  --release=<APP_RELEASE_VERSION>     required (e.g. v1.4.17)"
    echo ""
    echo "Example: ./build.sh --release=v1.4.17 --platform=linux/amd64"
    exit 1
fi
