...
There is no chart in this repository. The one on Artifact Hub is [plane-ce](https://artifacthub.io/packages/helm/makeplane/plane-ce), and installed as it comes it deploys images this repository does not build.
...
There is no `proxy` override because the chart has no proxy workload — it routes through an ingress instead, so `pace-proxy` has no counterpart in a Kubernetes install.
